...
beautiful.init("/home/ryadom/.config/awesome/themes/zenburn/theme.lua")
--config_dir = ("/home/ryadom/.config/awesome/")
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "Browser", "Code", "Term", "Vlc", 5, 6, "Games", 8, "Mail" }, s, layouts[1])
end
-- }}}
...
mymainmenu = awful.menu({ items = {
                                    { "programs", mymainmenu, beautiful.awesome_icon },
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ }, "XF86AudioMute", function ()
        if Muted then
		naughty.notify({
		    text = '<span font="12.5">Sound <span color="red">Unmuted</span></span>',
		    timeout = 0.5
		})
            awful.util.spawn("amixer -q sset Master unmute")
            Muted = false
        else
		naughty.notify({
		    text = '<span font="12.5">Sound <span color="red">Muted</span></span>',
		    timeout = 0.5
		})
            awful.util.spawn("amixer -q sset Master mute")
            Muted = true
        end
...
cmds_run_once = {
    {name = "nm-applet", cmd = "nm-applet"},
    {name = "chromium", cmd = "chromium"},
    {name = "volumeicon", cmd = "volumeicon"},
    {name = "thunderbird", cmd = "thunderbird"},
    {name = "lxpolkit", cmd = "/usr/lib/lxpolkit/lxpolkit"},
    {name = "gnome-screensaver", cmd = "gnome-screensaver"}
--    {name = "xflux", cmd = "xflux -l 50 -g 50"},
...
mytimer:connect_signal("timeout", function()
	...
	local handle = io.popen("acpi | grep '[0-9]\\{1,3\\}%' -o")
	local result = handle:read("*a")
	handle:close()
	mybatwidget:set_text(" " .. result .. " ")
...
for _,i in pairs(cmds_run_once) do
    awful.util.spawn_with_shell("pgrep -u $USER -x " .. i.name .. " || (" .. i.cmd .. ")")
end
